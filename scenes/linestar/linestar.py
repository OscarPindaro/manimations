...
class LineStar:
    ...
    grouped_lines: VGroup = field(default_factory=lambda: VGroup(), repr=False, init=False)  # Hidden from repr
    ...
    def _create_line(self, start: Tuple[float,float,float], end: Tuple[float,float,float], color: ManimColor | None = None) -> Line:
        if color is None:
...
class LinestarScene(Scene):
    ...
    def construct(self):
        ...
        self.wait(0.25)
        self.play(LaggedStart(*self.linestar.uncreate_lines(), lag_ratio=self.create_lag_ratio))