...
class ColorType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        # Check if the value exists in all_colors or all_manim_colors
            ...
            self.fail(f"'{value}' is not a valid hex color or known color. Expected a hex format like '#FFFFFF' or a known color name from COLORS or MANIM_COLORS.")
...
@click.option('--frame-rate', '-f', default=60, type=int, help="Frame rate of the output video.")
...
@click.option('--background_color', '-b', required=True, type=ColorType(), help="Background color (hex or named color).")
def render_linestar(width: int, height: int, frame_rate: float, output: str, palette: str, background_color: str):
    ...
    if not output:
        ...
        normalized_background = background_color.lstrip('#').lower().replace(" ", "_")  # If hex, strip the '#'
